...
try {
    const rows = db.prepare("SELECT * FROM streak_cache LIMIT 10").all();
    console.log(`Total rows in streak_cache: ${rows.length} (showing max 10)`);
    console.log(rows);

    // Also check history count to be sure we have data
    const historyCount = db.prepare("SELECT count(*) as count FROM activity_history").get();
    console.log(`Total history entries: ${historyCount.count}`);

} catch (error) {
    console.error("Failed to query streak_cache:", error.message);
}
