...
import { resolveOwnerAlias } from "../identity/owner-alias";
...
export const fetchSessions = async (
  ...
  const sessions = videos.map((video): PlexSession => {
    ...
    // Global Decision. Burned-in subtitles (subtitleDecision "transcode")
    // normally force videoDecision to "transcode" too, but Plex doesn't
    // guarantee it — fold them in explicitly, matching Tautulli.
    let finalDecision = "direct play";
    if (videoDecision === "transcode" || audioDecision === "transcode" || subtitleDecision === "transcode") {
      finalDecision = "transcode";
    } else if (videoDecision === "direct stream" || audioDecision === "direct stream") {
      finalDecision = "direct stream";
    }

    // Counters — three-way, Tautulli-style. Direct Stream is remux, not free.
    if (finalDecision === "transcode") transcoding += 1;
    else if (finalDecision === "direct stream") directStream += 1;
    else directPlay += 1;
    ...
    // Per-stream identity: Plex's sessionKey (falling back to Session.id, then
    // ratingKey as a last resort). The public `id` is the globally unique
    // stream id `${serverId}:${streamKey}` — two viewers of the same title, or
    // colliding ratingKeys across servers, never share an id.
    const streamKey = String(
      (video.sessionKey as string | number | undefined) ??
      video.Session?.id ??
      video.ratingKey ??
      crypto.randomUUID()
    );
    ...
    return {
      id: server?.id ? `${server.id}:${streamKey}` : streamKey,
      sessionKey: streamKey,
      sessionId: video.Session?.id as string | undefined,
      type: metadata.type || video.type,
      guid: (metadata.guid as string | undefined) || (video.guid as string | undefined),
      grandparentGuid:
        (metadata.grandparentGuid as string | undefined) ||
        (video.grandparentGuid as string | undefined),
      ratingKey: video.ratingKey ? String(video.ratingKey) : undefined,
      ...
      // Plex reports the server owner as local account id "1" — translate to
      // the real plex.tv accountId so stats/history/rules can match on it.
      userId: resolveOwnerAlias(server?.id, video.User?.id),
      ...
      secure: player.secure !== undefined ? String(player.secure) === "1" : undefined,
      progressPercent,
      duration,
      viewOffset,
      resolution,
