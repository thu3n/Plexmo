                    ...
                    const processBatch = withTransaction((entries: any[]) => {
                        for (const row of entries) {
                            try {
                                ...
                                const historyDup = hasHistoryNear(row.user, String(row.rating_key), startTimeMs - bufferMs, startTimeMs + bufferMs);
                                ...
                                const activeDup = hasActiveSessionNear(row.user, String(row.rating_key), startTimeMs - bufferMs, startTimeMs + bufferMs);
