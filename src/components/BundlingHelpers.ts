...
export function bundleHistoryEntries(entries: any[], options: { skipItemCheck?: boolean } = {}): BundleResult[] {
    ...
    for (let i = 1; i < entries.length; i++) {
        ...
        // Key on canonical accountId when present — display names are not
        // unique across servers. Fall back to name for pre-identity rows.
        const isSameUser = String(prev.userId ?? prev.user) === String(curr.userId ?? curr.user);
