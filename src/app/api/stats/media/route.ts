...
export async function POST(req: NextRequest) {
    try {
        // In-route guard: this route accepts client-supplied {serverId,
        // ratingKey} sources and fetches from those servers — it must never
        // rely on the middleware cookie gate alone.
        const user = await authorizeApiKeyOrSession(req);
        if (!user) {
        ...
        // Scoped viewers may only query servers inside their scope.
        for (const source of items.sources) {
            if (!canAccessServer(user.scope, source.serverId)) {
        ...
        if (items.type === 'movie') {
            ...
            history.forEach(h => {
                ...
                // Key on canonical accountId — the same person can have different
                // display names across servers (and names can collide).
                uniquePlayKeys.add(`${h.userId || h.user}-${date}`);
            });

            const totalPlays = uniquePlayKeys.size;
            const uniqueUsers = new Set(history.map(h => h.userId || h.user)).size;
        ...
        if (items.type === 'show') {
            ...
            const getStatsForKeys = (keys: { serverId: string, ratingKey: string }[]) => {
                ...
                return {
                    ...
                    uniqueUsers: new Set(relevantHistory.map(h => h.userId || h.user)).size,
            ...
            const generateUserStats = (keys: Set<string>) => {
                ...
                for (const h of enrichedHistory) {
                    ...
                    // Map key = canonical accountId (display name only as fallback);
                    // `user` stays the display name for the UI.
                    const userKey = String(h.userId || h.user);
                    if (!map.has(userKey)) {
                        map.set(userKey, {
                            user: h.user,
            ...
            seasons.forEach((season: any) => {
                ...
                seasonHistory.forEach((h: any) => {
                    ...
                    const keyPart = (h.season !== undefined && h.episode !== undefined)
                        ...
                        : h.ratingKey;
                    uniquePlayKeys.add(`${h.userId || h.user}-${keyPart}-${date}`);
                });

                season.stats = {
                    totalPlays: uniquePlayKeys.size,
                    uniqueUsers: new Set(seasonHistory.map(h => h.userId || h.user)).size,
            ...
            enrichedHistory.forEach(h => {
                ...
                const keyPart = (h.season !== undefined && h.episode !== undefined)
                    ? `S${h.season}E${h.episode}`
                    : h.ratingKey;
                globalUniquePlayKeys.add(`${h.userId || h.user}-${keyPart}-${date}`);
            });
            const finalTotalPlays = globalUniquePlayKeys.size;
            const finalUniqueUsers = new Set(history.map(h => h.userId || h.user)).size;
