...
import { PlexLinkCode } from "@/components/PlexLinkCode";

interface LoginStepProps {
    containerVariants: Variants;
    isAuthenticating: boolean;
    loginError: string;
    /** plex.tv/link device code — set when the installed-PWA flow is active. */
...
/**
 * Onboarding step 1: sign in with Plex. Triggers the OAuth flow via onLogin
 * (popup in browsers, plex.tv/link device code in installed PWAs); the parent
 * owns the auth state and polling.
 */
export default function LoginStep({ containerVariants, isAuthenticating, loginError, linkCode, onLogin, t }: LoginStepProps) {
    return (
        <motion.div
            key="step1"
