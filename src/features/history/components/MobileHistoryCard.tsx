...
import { formatDateTime, getPlayerIcon, getSplitDisplayTitle, useLivePause, historyThumbSrc } from "./HistoryHelpers";
    ...
    return (
        ...
        >
            ...
            <div className="relative shrink-0 w-16 h-24 rounded-xl overflow-hidden bg-black/40 shadow-lg ring-1 ring-white/10">
                {historyThumbSrc(entry, details) ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img loading="lazy" src={historyThumbSrc(entry, details)!} alt="" className="w-full h-full object-cover" />
                ) : (
                    <div className="flex h-full w-full items-center justify-center bg-white/5">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img loading="lazy" src={isTV ? "/images/libraries/show.svg" : "/images/libraries/movie.svg"} alt="" className="w-8 h-8 opacity-20" />
                    </div>
                )}
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/5 border border-white/5">
                        <div className="h-4 w-4 rounded-full overflow-hidden bg-white/10">
                            {details?.userThumb ? <img loading="lazy" src={details.userThumb} alt="" className="w-full h-full object-cover" /> : <div className="text-[8px] flex items-center justify-center font-bold h-full">{entry.user.slice(0, 1)}</div>}
                        </div>
...
export function MobileBundledCard({ bundle, timeZone, locale, isEditing, selectedIds, onToggle, onToggleGroup, onSelect }: BundledProps) {
    ...
    return (
        <div className="flex flex-col gap-1 min-w-0">
            <div
                className={clsx(
                    "relative flex items-center gap-4 p-4 rounded-3xl border bg-white/5 border-white/5 hover:bg-white/10 min-w-0 overflow-hidden",
            ...
            {isExpanded && (
                <div className="pl-4 space-y-2 border-l-2 border-white/5 ml-4 min-w-0">
                    {subEntries.map(sub => (
                        <div key={sub.id} className="scale-95 origin-left min-w-0">
                            <MobileHistoryCard
                                entry={sub}
                                timeZone={timeZone}
                                locale={locale}
