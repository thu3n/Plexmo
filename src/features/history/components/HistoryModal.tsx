...
import { formatAudioChannels, formatCodec, formatVideoRes, getPlayerIcon, getSplitDisplayTitle, DetailBadge, HoverReveal, formatDateTime, historyThumbSrc } from "./HistoryHelpers";
...
export function HistoryModal({ entry, onClose, onOpenStats }: {
    ...
    // Portaled to <body>: page containers create z-10 stacking contexts that
    // trapped the modal below the body-level global dock (z-50); z-[100]
    // clears the dock. Only ever rendered after a click, so document exists.
    return createPortal(
                    ...
                    <div className="flex flex-col sm:flex-row flex-1 min-h-0 w-full relative">
                        ...
                        <div className="relative w-full sm:w-[40%] shrink-0 overflow-hidden border-b sm:border-b-0 sm:border-r border-white/5 h-48 sm:h-auto min-h-[200px] bg-black/20 z-0">
                            {historyThumbSrc(entry, details) ? (
                                <>
                                    <img
                                        src={historyThumbSrc(entry, details)!}
                                        alt={mainTitle}
                                        onError={(e) => {
                        ...
                        <div className="pl-4 shrink-0">
                            ...
                            >
                                <div className="h-10 w-10 rounded-full ring-2 ring-white/10 overflow-hidden shadow-lg transition-transform hover:scale-105 hover:ring-amber-500/50">
                                    <img
                                        src={avatarSrc(details?.userThumb, entry.user)}
                                        alt={entry.user}
                                        loading="lazy"
