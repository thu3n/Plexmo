...
import { Plus, Trash2, Edit2, X, Eye, EyeOff, Copy, Check } from "lucide-react";
                            ...
                            <SettingsCard key={server.id} className="group relative flex flex-col justify-between min-h-[200px]">
                                <div>
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        {/* Always visible on touch/small screens (no hover); reveal-on-hover only on desktop */}
                                        <div className="flex gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => handleEdit(server)} aria-label={t("common.edit")} className="p-2.5 hover:bg-white/10 active:bg-white/20 rounded-lg text-white/70 hover:text-white">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(server.id, server.name)} aria-label={t("settings.confirmDelete")} className="p-2.5 hover:bg-rose-500/20 active:bg-rose-500/30 rounded-lg text-rose-400 hover:text-rose-300">
...
function ServerModal({ isOpen, onClose, server, onSuccess }: { isOpen: boolean, onClose: () => void, server: PublicServer | null, onSuccess: () => void }) {
    ...
    // Token reveal/copy (raw token is fetched on demand from the authenticated endpoint)
    const [showToken, setShowToken] = useState(false);
    const [revealedToken, setRevealedToken] = useState<string | null>(null);
    const [justCopiedToken, setJustCopiedToken] = useState(false);

    // Lazily fetches the full token for an existing server and swaps it into the form.
    const ensureRealToken = async (): Promise<string | null> => {
        if (revealedToken) return revealedToken;
        if (!server) return formData.token; // new server: token is already raw
    ...
    const handleToggleToken = async () => {
        if (!showToken) await ensureRealToken();
        setShowToken(v => !v);
    };
    ...
    const handleCopyToken = async () => {
        ...
        setTimeout(() => setJustCopiedToken(false), 2000);
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200 overflow-y-auto">
            <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-slate-900 p-6 sm:p-8 shadow-2xl relative my-auto max-h-[90dvh] overflow-y-auto">
                        ...
                        <div>
                            ...
                            <div className="flex items-stretch gap-2">
                                <input
                                    ...
                                    onChange={e => { setFormData({ ...formData, token: e.target.value }); setRevealedToken(e.target.value); }}
                                ...
                                {server && (
                                    <button
                                        ...
                                        className="px-4 bg-white/5 hover:bg-white/15 border border-white/10 rounded-xl transition-colors text-white/70 active:scale-95"
                                    >
                                ...
                                <button
                                    ...
                                    className="px-4 bg-white/5 hover:bg-white/15 border border-white/10 rounded-xl transition-colors text-white/70 active:scale-95"
                                >
                                    {justCopiedToken ? <Check className="w-5 h-5 text-emerald-400" /> : <Copy className="w-5 h-5" />}
                                </button>
                            </div>
                        </div>
